...
const welcomeEvent = require('./erlc/command/events/welcome');
welcomeEvent(client);
...
try {
    if (!fs.existsSync('./blacklist.json')) {
        ...
        fs.writeFileSync('./blacklist.json', JSON.stringify(defaultBlacklist, null, 4));
...
const commandLogger = require('./erlc/command/events/commandLogger');
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isCommand()) {
        ...
        if (interaction.customId === 'enter_giveaway') {
            const giveawayCommand = require('./slashcommands/command/giveaway.js');
            try {
